...
high-availability block-level storage system with automatic “online”
block deduplication. Extensive caching capability is integrated into
both the client and server components to dramatically improve
...
Intel IPP: Commercial license but free for development, and royalty-free
usage. Provides high-performance compression, hash computation and
...
There are two distinct modes in which the server can run. The mode is
controlled by the definition of a macro called **ULTRABAC\_CLOUD\_USE**
which is not defined by default, but the UB\_Debug/UB\_Release project
configurations of the server project define this macro. For want of a
better term, the default project configuration will be referred to as a
“Local configuration” and when the macro is defined the configuration
will be referred to as a “Global configuration. The differences between
behavior is outlines below.

Local Configuration
-------------------

-   This configuration is designed for the server to run on a secure
    local network, or over a secure VPN where the server is hosted in
    the cloud
...
-   No authentication is used or needed – clients identify/configure
    themselves to the server based on their computer name.
...
-   Client computers can be manually configured by using registry
    ...
    mode that allows each client computer to receive their credential
    configuration from a server on their local network
...
-   Because the server may be exposed to the public internet, some
    hardening against malevolent actors is built-in. Any unexpected
    communication data or a failed authentication attempt results in
    that IP address being placed in a “banned” IP address database and
    no communication is possible with that IP address until a
    configurable period of time has elapsed. Currently the timeout
    period is short for testing purposes, but in real-world deployments
    should be set to 10 minutes or more.
...
-   The server verifies each request to retrieve a common-file by
    ...
    encountered the full file at some point.

Method of Operation (from Client Perspective)
=============================================

Although not completely current, please review the Visio files
.\\GENSHA1\\Flowcharts.vsd and .\\GENSHA1\\DataModel.vsd for reference

Review the GENSHA1 / Linux\_GENSHA1 project as the primary references as
these command-line projects expose virtually all functionality of the
...
fail then the variable should be released and an error reported.
...
-   GCFOS\_SRV\_RESP\_RESIDENT: This file is known and already present
    ...
    the future, all that will be needed is the hash, filesize and
    verification key (a total of 28 bytes). Note that the verification
    key is only necessary on GCFOS servers in global-configuration mode.
...
3\. To store a buffer of data in the block store directly, call
StoreBlocks(). To retrieve that data later, call RetrieveBlocks(). Note
that the maximum number of blocks that may be stored or retrieved may
not exceed GCFOS\_BLOCKS\_PER\_QUERY (16) which means that the buffer
size may not exceed 64KB.
...
Extended cache allows the retrieve the complete hash chain for any file
already stored in the block store by GetBlockHashesForFile() . This
means that to “backup” an entire file that is already stored in the
block store can be achieved in typically 50us to 1.0ms. The performance
increase of these cache options can increase throughput 50 to 200 times.
The cost in disk space for these cache options is approximately 1%
(each) of the size of the data stored in the block store. For example,
if the client has stored 1TB of data in the block store and extended
caching has been enabled, the client’s local cache database will occupy
at least 2GB of local disk space.
...
The sub-project ConfigureDeduplicationServer provides a UI for
...
GCFOS server on a virtual machine on that same cloud provider can
improve performance and reduce data-transfer costs that may be levied by
the cloud provider.
...
has registered. For “local mode” use, this is merely a simply
...
stored previously, and if so what file sequence\# the data resides in.
...
the server, the probability that there is a collision will be 10,000,000
\^ 2 / 2 \* 2 \^ 170 which is \~ 3 x10\^-38 (the odds of a meteor
landing on your house are much higher).

Block store design
==================

The block store will only be useful in the case where data is
consistently 4KB-aligned (or multiples thereof) so that blocks
containing the same data can be detected. The typical use-case would be
“image backup” where data blocks are always aligned on sector boundaries
(usually 0x200 or 0x1000), or the backup of traditional file data where
contents remain the same, without insertions/deletions causing the data
to have different new alignments. Data being written at the end of the
data, appending to existing data, is handled efficiently since only the
new blocks added will need to be stored.
...
contains more than 10,000 files and so efficiently uses NTFS (or other
file system). Block data is written compressed into this file, which
means that the offset needs to be stored for each block reference in the
“blocks” database. Blocks are all 0x1000 bytes in size, and are
...
when hashchains are used to represent the common files.
...
(eg 60 days, or other configurable value) it will transfer those limbo
...
of the CPU is performed by the clients, effectively making this a
...
and click the “Configure Redirection Mode” button on the main screen.
This would then present the following dialog box, which would allow the
credentials to be entered. Then, whenever a GCFOS client attempts to
auto-configure, it will receive the credentials from the server and make
the registry entries necessary automatically.

![](media/image1.png){width="6.104166666666667in" height="3.3125in"}
...
file they wish to download but have never owned, and makes an attempt to
retrieve it from the server. As part of the retrieval request a 4-byte
client-unique “validation key” is included. Although a deterministic
process is used to set/check the validation key, it requires access to
the first 1MB of file data. So, the actor would need access to the first
1MB of file data in order to calculate the validation key. If they had
the file already in order to calculate this key they wouldn’t need to
retrieve it in the first place. Guessing the validation key incorrectly
results in the IP address being added to the “banned” list, the session
closed, and a wait period of 10 minutes or so before being unbanned. A
brute force attack would therefore require an average 147 million years.
