...
class btree_node {
 ...
 public:
  ...
  void set_position(int v) { fields_.position = (Params::node_count_type)v; }  //PAB

  // Getter/setter for the number of values stored in this node.
  int count() const { return fields_.count; }
  void set_count(int v) { fields_.count = (Params::node_count_type)v; } //PAB
  ...
  void set_child(int i, btree_node *c) {
    *mutable_child(i) = c;
    ...
    c->fields_.position = (Params::node_count_type)i; //PAB
  }
  ...
  static btree_node* init_leaf(
    ...
    f->leaf = 1;
    f->position = 0;
    f->max_count = (Params::node_count_type)max_count; //PAB
    f->count = 0;
    f->parent = parent;
    if (!NDEBUG) {
